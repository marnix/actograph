...
13. **Multi-device sync** — Add a `merge` command that loads a second `.automerge` file and merges it into the local one. Must handle duplicate slugs and duplicate tag actions that can arise from independent creation on different devices (currently detected and rejected at load time; merge should auto-resolve by regenerating slugs and renaming tag actions)
